/**
 * @file make_sample_h5.cpp
 * @brief Extract a structured i=0 slice into a compact sample HDF5 file.
 */
#include <hdf5.h>
...
/** @brief Throw when an HDF5 operation reports failure. */
void check(herr_t status, const char *operation) {
  if (status < 0)
    throw std::runtime_error(operation);
}

/** @brief Return a valid HDF5 identifier or throw. */
...
/** @brief Write one signed 64-bit root attribute. */
void write_attribute(hid_t file, const char *name, std::int64_t value) {
  const hid_t space =
      require_id(H5Screate(H5S_SCALAR), "create attribute dataspace");
...
/** @brief Return and validate the five-dimensional source dataset shape. */
std::array<hsize_t, 5> dataset_shape(hid_t dataset, const char *name) {
  const hid_t space =
      require_id(H5Dget_space(dataset), "get dataset dataspace");
...
/** @brief Copy the first streamwise plane into a compact output dataset. */
void copy_i0_slice(hid_t input, hid_t output, const char *source_name,
  ...
  const hid_t source_space =
      require_id(H5Dget_space(source), "get source dataspace");
  check(H5Sselect_hyperslab(source_space, H5S_SELECT_SET, offset.data(),
                            nullptr, count.data(), nullptr),
        "select i=0 hyperslab");

  // Structured sample layout: [k, j, dof, complex parts].
  const std::array<hsize_t, 4> output_shape{shape[0], shape[1], shape[3],
                                            shape[4]};
  const hid_t output_space =
      require_id(H5Screate_simple(static_cast<int>(output_shape.size()),
                                  output_shape.data(), nullptr),
                 "create output dataspace");
  const hid_t destination = require_id(
      H5Dcreate2(output, destination_name, H5T_IEEE_F64LE, output_space,
                 H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT),
      "create output dataset");
  const std::size_t value_count =
      static_cast<std::size_t>(shape[0] * shape[1] * shape[3] * shape[4]);
  std::vector<double> values(value_count);
  check(H5Dread(source, H5T_NATIVE_DOUBLE, output_space, source_space,
...
/** @brief Parse input/output paths and extract the structured sample. */
int main(int argc, char **argv) {
  ...
  try {
    input = require_id(H5Fopen(argv[1], H5F_ACC_RDONLY, H5P_DEFAULT),
                       "open input file");

    const hid_t flow =
        require_id(H5Dopen2(input, "flow", H5P_DEFAULT), "open flow");
    ...
    output = require_id(
        H5Fcreate(temporary.c_str(), H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT),
        "create output file");
